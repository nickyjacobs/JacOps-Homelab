...
## Wiki: Homelab (lazy lookup, niet @import, optioneel)

Als er een lokale Homelab-wiki beschikbaar is (privé, buiten deze repo),
configureer het pad in `CLAUDE.local.md` (gitignored) zodat het als
kennisbron gebruikt kan worden zonder dat het pad in de publieke repo staat.
De wiki bevat concrete configs, echte IPs en architectuur-details die hier
nooit terecht mogen komen.
...
door placeholders** (zie @.claude/rules/security-first.md).

## Regels

- @.claude/rules/writing-style.md — DutchQuill, NL leidend + EN mirror, toon
- @.claude/rules/security-first.md — Placeholder-discipline, Vaultwarden refs
- @.claude/rules/commit-policy.md — Nicky commit zelf, geen trailers
- @.claude/rules/repo-conventions.md — Naamgeving, frontmatter, mappen, roadmap-bewustzijn
...
Zie `.claude/hooks/`. Vijf hooks, geconfigureerd in `.claude/settings.json`:
...
## Commit policy (samenvatting, details in .claude/rules/commit-policy.md)
...
- **Nooit** `git commit`, `git push`, `git push --force`, `git reset --hard`,
  ...
  `.claude/settings.json` op de **deny**-lijst
