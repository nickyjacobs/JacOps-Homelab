...
## WebAuthn does not work with IP addresses as Relying Party ID

The Proxmox VE Auto-fill for WebAuthn Settings populates the node IP address as the RP ID and Origin. That is technically correct from PVE's perspective, but the WebAuthn spec and most browser implementations require a domain name as RP ID. The result was a non-descriptive error: `failed to begin webauthn context instantiation: The configuration was invalid`.

The fix was to set up local DNS records via the UniFi gateway (Client Devices > device > IP Settings > Local DNS Record) and update the PVE WebAuthn Settings with the hostname as RP ID and Origin.

**Takeaway:** always access services via hostname, not IP. Set up local DNS as the first step on a new cluster, before starting any 2FA configuration. The error message from PVE does not point to the cause, which makes debugging harder than it needs to be.

## Firefox enterprise_roots does not import self-signed end-entity certs

`security.enterprise_roots.enabled = true` in Firefox only imports certificates with the CA basic constraint from the macOS system keychain. Individual self-signed certs without the CA flag are ignored, even when added via `sudo security add-trusted-cert -d -r trustRoot`. Chrome and Safari trust these certs directly through the system keychain.

The result was that a self-signed cert for PVE worked in Chrome but not in Firefox, despite the cert being listed as trusted in the system keychain.

**Takeaway:** for multi-browser certificate trust on macOS, a homelab CA is the right approach. The CA has the CA basic constraint, gets imported by Firefox via enterprise_roots, and every cert signed by it is automatically trusted across all browsers.

## macOS passkey handler intercepts Firefox WebAuthn with hardware keys

Firefox on macOS delegates WebAuthn by default to the macOS passkey handler (`security.webauthn.enable_macos_passkeys = true` in `about:config`). That handler first shows a "Save a passkey?" dialog for iCloud Keychain. After "More Options" and "Security Key", the hardware key should be reachable, but in practice the macOS handler did not recognize the YubiKey touch. Chrome, which uses its own WebAuthn implementation, worked immediately.

The fix was setting `security.webauthn.enable_macos_passkeys` to `false`. Firefox then uses its own FIDO2/WebAuthn handler that communicates directly with the key via USB HID, without the macOS intermediary.

**Takeaway:** set this preference to `false` in Firefox if you use a hardware security key. The macOS passkey handler is designed for iCloud Keychain passkeys and does not work reliably with USB security keys in Firefox.

## Docker containers in LXC do not trust the host CA

The homelab CA was trusted in the system store of CT 161 (the Forgejo Runner LXC). But Docker containers that the runner starts for CI jobs are isolated: they have their own `/etc/ssl/certs/` and know nothing about the host CA. The `actions/checkout` step failed with `server certificate verification failed` when cloning the repo via HTTPS from Forgejo.

The fix is two volume mounts in the runner config:

- Mount the homelab CA cert as `/etc/ssl/certs/homelab-ca.crt` in the container
- Mount the full host CA bundle (`/etc/ssl/certs/ca-certificates.crt`, which includes the homelab CA after `update-ca-certificates`) to replace the container bundle

Additionally, set `NODE_EXTRA_CA_CERTS` as an environment variable so Node.js (which the checkout action runs on) adds the homelab CA to its trust list.

**Takeaway:** each layer of isolation has its own cert store. The LXC host, Docker container and the application inside the container can each trust a different set of CAs. With self-signed or private CA setups, the CA must be made available at every layer separately.

## SSL_CERT_FILE replaces the entire CA store

When configuring TLS trust for the Forgejo Runner, I set `SSL_CERT_FILE` to the path of the homelab CA cert. The idea was that tools would read this file and trust the CA. What I did not expect: `SSL_CERT_FILE` replaces the entire CA store. Every application that reads this variable (curl, git, lychee, and most other tools) then trusts only the CAs in that file. Public CAs like those for shields.io, LinkedIn and other external sites were no longer trusted.

The fix was removing `SSL_CERT_FILE` and using `NODE_EXTRA_CA_CERTS` instead. That variable is Node.js-specific and adds the CA to the existing list rather than replacing it. For non-Node tools, the mounted `ca-certificates.crt` bundle that already contains the homelab CA does the job.

**Takeaway:** `SSL_CERT_FILE` is an override, not an addition. Only use it when you want to replace the entire bundle. For adding a single CA to an existing trust store, better options exist: `NODE_EXTRA_CA_CERTS` for Node.js, or adding the CA to the system bundle via `update-ca-certificates` and mounting that bundle.

## GitHub Action wrappers are not always compatible with Forgejo Actions

Forgejo Actions is compatible with the GitHub Actions workflow format, but that does not automatically extend to the actions themselves. Two out of two tested actions failed:

- `gitleaks/gitleaks-action@v2` requires a paid license outside GitHub. On GitHub it works for free on public repos through a special integration. On Forgejo it is treated as a standalone action and demands a license key
- `lycheeverse/lychee-action@v2` downloads the lychee binary but the path is not correctly passed through in the act runner. The binary exists but is not discoverable via PATH

The fix was installing the tools as CLI binaries directly in the workflow instead of using the action wrappers. Both tools are free and open source; the wrappers only add convenience that does not work outside GitHub.

**Takeaway:** test every GitHub Action you migrate to Forgejo individually. Composite actions and actions that call GitHub-specific APIs (code scanning, annotations, issue creation) are the most likely breaking points. CLI-based workflows are more portable than action-based workflows.

## Cross-VLAN is fully closed, including Servers to Apps

During the Beszel deploy, the assumption was that PVE nodes (Servers VLAN) could reach the Apps VLAN. The decisions doc states that "the hypervisors can reach the Apps subnet for container management". That is true for the management plane: `pct exec` works through a local Unix socket on the hypervisor, not through the network. For TCP traffic over the network (such as a Beszel agent connecting via WebSocket to the hub), traffic is blocked by the zone firewall in both directions.

The fix was a targeted UniFi firewall policy: source device-based (only the two PVE nodes), destination a specific IP (CT 151) on a specific port (8090 TCP). That is the narrowest possible opening without breaking the zone model.

**Takeaway:** `pct exec` does not prove that network traffic can flow between two VLANs. The management plane and the data plane are separate paths. Always test with an actual TCP connection (curl, ping) before assuming cross-VLAN communication works.

## DNS record must exist before a Traefik ACME request

The Traefik dynamic config for `beszel.jacops.local` was created before the DNS record existed in UniFi. Traefik loaded the route and triggered an ACME request to step-ca. That request uses tls-alpn-01, which requires step-ca to resolve the hostname and connect on port 443. Without a DNS record, the challenge failed, and Traefik fell back to its default certificate (`CN=TRAEFIK DEFAULT CERT`).

Traefik does not automatically retry failed ACME requests. The result was a working route with an invalid certificate. Only after a Traefik restart was the request re-triggered and succeeded.

**Takeaway:** when adding a new Traefik route, always create the DNS record first, then the dynamic config. If the order was reversed (as happened here), restart Traefik after creating the DNS record to trigger the ACME request.

## Shoutrrr ntfy auth requires the token as password, not username

Beszel uses Shoutrrr for webhook notifications. The ntfy URL format `ntfy://token@host/topic` places the token in the username field. ntfy does not recognise that as authentication and rejects the message with an auth error.

The correct format is `ntfy://:token@host/topic` with a colon before the token. The empty username and the token as password causes Shoutrrr to send it as a Bearer token in the Authorization header.

**Takeaway:** when configuring a Shoutrrr URL for ntfy, note the colon before the token. The difference between `ntfy://tk_abc@host/topic` (wrong) and `ntfy://:tk_abc@host/topic` (correct) is a single character that determines whether authentication fails or notifications arrive.
